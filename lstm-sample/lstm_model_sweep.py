...
default_config = {
    "epochs": 500,
    "patience": 50,  # Number of epochs early stop will wait for for improvement in validation loss
    "delta": 0.002,  # Minimum number validation loss needs to improve by
    "dataset": "",
    "variables": [],
    "train_size": 0,
    "input": 0,
...
sweep_config = {
    ...
    "parameters": {
        ...
        "layer": {"values": [1]},
        "dropout": {"values": [0.5]},
        "learning_rate": {"values": [0.00001, 0.00003, 0.00006, 0.0001]},
        "lambda_l1": {"values": [0.00000001]},
        "lambda_l2": {"values": [0.00000001]},
    },
}
...
def train(config):
    def train_inner(config=None):
        ...
        # Set to online if want wandb

        run = wandb.init(config=config, group="mac_1-15", mode="online")
        ...
        # Set run name
        run_name = f'{config["dataset"][5:-4]}, {config["look_back"]}, \
...
for dataset in datasets:
    ...
    sweep_id = wandb.sweep(sweep_config, project="AHU-sweep")
